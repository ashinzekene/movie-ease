...
export class MoviesApi {
  ...
  one(id) {
    return this.http.get(`http://movie-ease.herokuapp.com:5000/movies/one/${id}`).map((res)=> {
      return JSON.parse(res.json())
    }).catch(this._handleError)
  }
  search(query) {
    return this.http.get(`http://movie-ease.herokuapp.com:5000/movies/search/${query}`).map((res)=> {
      return JSON.parse(res.json())
    }).catch(this._handleError)
  }
  popular(n=1) {
    console.log("Getting popular")
    return this.http.get('http://movie-ease.herokuapp.com:5000/movies/popular/'+n).map((res)=> {
  ...
  latest(n=1) {
    ...
    return this.http.get('http://movie-ease.herokuapp.com:5000/movies/latest/'+n).map((res)=> {
  ...
  upcoming(n=1) {
    ...
    return this.http.get('http://movie-ease.herokuapp.com:5000/movies/upcoming/'+n).map((res)=> {
  ...
  topRated(n=1) {
    ...
    return this.http.get('http://movie-ease.herokuapp.com:5000/movies/top-rated/'+n).map((res)=> {
